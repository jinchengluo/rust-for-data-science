...
#[pyfunction]
#[pyo3(signature = (x, y, test_size=0.2, train_size=None))]
fn stratified_train_test_split<'py>(
    ...
    let labels: Vec<i32> = {
        ...
        let unique_set: HashSet<i32> = y_array.iter().cloned().collect();
    ...
    for i in 0..n_samples {
        let label = y_array[i];
        let sample: Vec<f64> = (0..n_features).map(|j| x_array[[i, j]]).collect();
